<?php //Note that ticket is initiated in tickets.php.
...
<table width="100%" cellpadding="2" cellspacing="0" border="0">
    <tr>
        <td class="msg" width=50%>
            Ticket #<?php echo $ticket->getExtId()?>&nbsp;<a href="tickets.php?id=<?php echo $id?>" title="Reload"><span class="Icon refresh">&nbsp;</span></a></td>

        <td class="msg" width=50%>
            <?php  if($thisuser->canEditTickets() || ($thisuser->isManager() && $dept->getId()==$thisuser->getDeptId())) { ?>
             <a href="tickets.php?id=<?php echo $id?>&a=edit" title="Edit Ticket" class="Icon editTicket">Edit Ticket</a>
            <?php }?>
        </td>
    </tr>
    <tr>
    ...
    <tr><td colspan=2 class="msg">Subject: <?php echo Format::htmlchars($ticket->getSubject())?></td></tr>
    <tr>
        ...
        <form name=action action='tickets.php?id=<?php echo $id?>' method=post class="inline" >
            ...
            <select id="do" name="do" 
              onChange="this.form.ticket_priority.disabled=strcmp(this.options[this.selectedIndex].value,'change_priority','reopen','overdue')?false:true;">
                ...
                <option value="change_priority" <?php echo $info['do']=='change_priority'?'selected':''?> >Change Priority</option>
                <?php if(!$ticket->isoverdue()){ ?>
                <option value="overdue" <?php echo $info['do']=='overdue'?'selected':''?> >Mark Overdue</option>
                <?php }?>
                ...
                <?php                  if($thisuser->canManageBanList()) {
                    if(!$emailBanned) {?>    
...
?>
<div align="left">
    <a class="Icon note" href="#" onClick="toggleLayer('ticketnotes'); return false;">Internal Notes (<?php echo $notes?>)</a><br><br>
                         ...
                         if($appendStaffSig || $appendDeptSig) { ?>
                          <div style="margin-top: 10px;">
                               ...
                               <label> <input type="radio" name="signature" value="mine" <?php echo $info['signature']=='mine'?'checked':''?> > My signature</label>
                                ...
                                <label><input type="radio" name="signature" value="dept" <?php echo $info['signature']=='dept'?'checked':''?> > Dept Signature</label>
                                <?php }?>
                           </div>
                         <?php }?>
                        <div style="margin-top: 3px;">
                            ...
                            if($ticket->isOpen()){?>
                            <label><input type="checkbox" name="ticket_status" id="l_ticket_status" value="Close" <?php echo $checked?> > Close on Reply</label>
                            <?php }else{ ?>
                        ...
                        <div>
                            ...
                            <select id="dept_id" name="dept_id">
                                ...
                                while (list($deptId,$deptName) = db_fetch_row($depts)){
                                    ...
                                    <option value="<?php echo $deptId?>"<?php echo $selected?>><?php echo $deptName?> Department </option>
                                <?php 
                                }?>
